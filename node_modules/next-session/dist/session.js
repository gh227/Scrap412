...
class Session {
    constructor(req, res, sess) {
        ...
        if (sess) {
            ...
            this.cookie = new cookie_1.default(sess.cookie);
        }
        else {
            this.cookie = new cookie_1.default(req._sessOpts.cookie);
        }
    }
    //  touch the session
    touch() {
    ...
    async commit() {
        ...
        let touched = false;
        let saved = false;
        const shouldSave = () => core_1.stringify(this) !== this.req._sessStr;
        const shouldTouch = () => {
            if (!this.cookie.maxAge || !this.cookie.expires || touchAfter === -1)
                return false;
            const elapsed = this.cookie.maxAge * 1000 -
        ...
        if (shouldSetCookie()) {
            if (this.res.headersSent)
                return;
            const sessionId = typeof this.req._sessOpts.encode === 'function'
