...
function promisifyStore(store) {
    store.get = util_1.promisify(store.get);
    store.set = util_1.promisify(store.set);
    store.destroy = util_1.promisify(store.destroy);
    if (typeof store.touch === 'function')
        store.touch = util_1.promisify(store.touch);
    return store;
}
